...
import { Star, Trophy, LogOut, RefreshCw, Search, Users, Calendar, Filter, BarChart3, Eye, X, ChevronDown, Play, Clock, MapPin, TrendingUp, Flame } from 'lucide-react'
...
// Nouveau composant de carte moderne
function ModernMatchCard({ match, onRate, currentUserId }: {
  match: Match
  ...
  return (
    <div 
      ...
      className="group relative bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer overflow-hidden border border-gray-200 dark:border-slate-600 hover:border-indigo-300 dark:hover:border-indigo-600 transform hover:-translate-y-2"
    >
      {/* Sport Badge */}
      ...
      <div className="p-6 pb-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div className={`w-10 h-10 bg-gradient-to-r ${getSportGradient(match.sport)} rounded-xl flex items-center justify-center shadow-lg`}>
        ...
        <div className="mb-4">
          {match.sport === 'f1' ? (
            <div className="text-center bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 rounded-xl p-4">
            ...
            <div className="bg-gradient-to-r from-gray-50 to-blue-50 dark:from-slate-700/50 dark:to-blue-900/20 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3 flex-1 min-w-0">
                  {match.homeTeamLogo && (
                    <img src={match.homeTeamLogo} alt="" className="w-8 h-8 rounded-lg shadow-sm" />
                  )}
                ...
                <div className="flex items-center space-x-3 flex-1 justify-end min-w-0">
                  ...
                  {match.awayTeamLogo && (
                    <img src={match.awayTeamLogo} alt="" className="w-8 h-8 rounded-lg shadow-sm" />
                  )}
                </div>
              </div>
            </div>
      ...
      <div className="px-6 pb-6">
        {!match.canRate ? (
          <div className="text-center py-4 bg-gray-100 dark:bg-slate-700 rounded-xl">
            <Clock className="w-6 h-6 text-gray-400 mx-auto mb-2" />
            <div className="text-sm text-gray-500 dark:text-gray-400">
              Événement pas encore disponible pour notation
            </div>
          </div>
        ) : userRating ? (
          <div className={`bg-gradient-to-r ${getSportGradient(match.sport)} rounded-xl p-4 text-white`}>
            ...
            {userRating.comment && (
              <p className="text-sm text-white/90 italic">"{userRating.comment}"</p>
            )}
          </div>
        ) : (
          <div onClick={(e) => e.stopPropagation()}>
            {!showRatingForm ? (
              <button
                ...
                className={`w-full bg-gradient-to-r ${getSportGradient(match.sport)} text-white py-3 px-4 rounded-xl font-semibold transition-all duration-200 active:scale-95 shadow-lg hover:shadow-xl flex items-center justify-center space-x-2 group-hover:shadow-2xl`}
              >
                <Star className="w-5 h-5" />
                <span>Noter cet événement</span>
              </button>
            ) : (
              <div className="space-y-4 bg-gray-50 dark:bg-slate-700/50 rounded-xl p-4">
                <div>
                  <p className="text-sm font-semibold mb-3 text-gray-900 dark:text-white">Votre note :</p>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-3 border-2 border-gray-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all resize-none bg-white dark:bg-slate-800 text-gray-900 dark:text-white"
                    rows={3}
                ...
                <div className="flex space-x-2">
                  <button
                    ...
                    className={`flex-1 py-3 px-4 rounded-xl font-semibold transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed bg-gradient-to-r ${getSportGradient(match.sport)} text-white shadow-lg hover:shadow-xl`}
                  >
                    {isRating ? (
                  ...
                  <button
                    ...
                    className="px-4 py-3 bg-gray-200 dark:bg-slate-600 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-300 dark:hover:bg-slate-500 transition-all duration-200 active:scale-95"
                  >
                    ✕
                  </button>
                </div>
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/0 to-purple-500/0 group-hover:from-indigo-500/5 group-hover:to-purple-500/5 transition-all duration-300 pointer-events-none rounded-2xl"></div>
